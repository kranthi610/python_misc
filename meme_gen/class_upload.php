...
class Upload{
    ...
    public function doUploads($infiles=[],$target_dir=""){
		...
		$filename = $upfile["name"];
                $result = $this->add_to_db($time,$imageFileType,$filename);
                if($result['success']){
    ...
    private function add_to_db($time,$type,$filename,$uid=0){
        $dir = $this->targetDir;
        $thumbs = $this->targetThumbs;
        $ip = $_SERVER['REMOTE_ADDR'];
        $sql = "INSERT INTO `images` VALUES (NULL, {$uid}, '{$time}', '{$type}', '{$time}', '{$dir}', '{$thumbs}', '', '{$ip}','{$filename}');";
